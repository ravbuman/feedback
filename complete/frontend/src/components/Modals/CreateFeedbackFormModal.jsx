...
import { X, Plus, Trash2, FileText, BarChart3, Loader2, Move, GripVertical, Users, Globe } from 'lucide-react';
import { adminAPI } from '../../services/api';
import toast from 'react-hot-toast';

const CreateFeedbackFormModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      trainingName: '',
      assignedFaculty: [],
      activationStart: new Date().toISOString().slice(0, 16), // Current date/time
      activationEnd: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 16), // 30 days from now
      isActive: true
    }
  });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await adminAPI.createFeedbackForm(formData);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-gray-900">Create Feedback Form</h3>
              <p className="text-sm text-gray-500">Build a custom feedback form with questions</p>
            </div>
          </div>
          <button
            onClick={handleClose}
          ...
          <div className="bg-gray-50 rounded-lg p-4 space-y-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <label className="text-sm text-gray-700">
                Activate form immediately after creation
              </label>
            </div>
          </div>
...
export default CreateFeedbackFormModal;
